...
1. **窗口焦点**：确保微信窗口处于激活状态，否则第一次点击只会激活窗口
2. **搜索延迟**：搜索联系人后需要等待结果加载，建议 wait 500ms
3. **消息发送**：默认 enter 发送，如果用户修改了设置可能需要点击发送按钮
4. **表情符号**：可以直接在消息中使用 emoji，微信会正确显示
5. **@提及**：在群聊中 @ 某人，输入 `@` 后会弹出成员列表
6. **图片识别**：聊天区域的消息气泡，左侧是对方消息，右侧是自己消息
7. **未读消息**：会话列表中有红点或数字标记的是未读消息
