...
const NOTIF_WATCH_PATH = join(__dirname, '..', '..', '..', 'native', 'windows', 'notif-watch', 'bin', 'release', 'NotifWatch.exe')
...
export class WindowsNotificationProvider implements NotificationProvider {
  ...
  private spawn(): void {
    ...
    if (!existsSync(NOTIF_WATCH_PATH)) {
      console.error(`[NotificationProvider:win32] binary not found: ${NOTIF_WATCH_PATH}`)
      return
    }
    ...
    rl.on('line', (line: string) => {
      ...
      try {
        ...
        if (data.status) {
          console.log(`[NotificationProvider:win32] ${data.status} (pid: ${data.pid})`)
          return
        }
    ...
    this.process.stderr?.on('data', (chunk: Buffer) => {
      console.error(`[NotificationProvider:win32] stderr: ${chunk.toString().trim()}`)
    })
